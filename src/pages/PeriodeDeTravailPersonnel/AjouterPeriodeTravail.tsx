...
const AjouterPeriodeTravail = () => {
  document.title = "Ajouter Période De Travail | ENIGA";

  const [dateDebut, setDateDebut] = useState("");
  const [dateFin, setDateFin] = useState("");
  const [debutJourney, setDebutJourney] = useState("");
  const [finJourney, setFinJourney] = useState("");
  const [debutPause, setDebutPause] = useState("--:--");
  const [finPause, setFinPause] = useState("--:--");

  const [createWorkingDay] = useAddPersonnelWorkingDayMutation();

  const notifySuccess = () => {
    Swal.fire({
      position: "center",
      icon: "success",
      title: "La période a été créée avec succès",
  ...
  const onSubmitWorkingDay = (e: React.FormEvent<HTMLFormElement>) => {
    ...
    try {
      const periodeData = {
        ...
        period_end: dateFin,
        part_time: daily_pause_start === "--:--" ? "yes" : "no",
      };
      createWorkingDay(periodeData)
        .then(() => notifySuccess())
    ...
    <React.Fragment>
      <div className="page-content">
        <Container fluid={true}>
          <Breadcrumb
            title="Ajouter Période De Travail"
...
export default AjouterPeriodeTravail;
