...
const ListeAvisCommission = () => {
  document.title = "Liste Avis de Commission | ENIGA";
  ...
  function tog_AjouterCommission() {
    navigate("/directeur-de-stage/ajouter-avis-de-commission");
  }

  const { data = [] } = useFetchAllAvisCommissionQuery();
  const [deleteAvisCommission] = useDeleteAvisCommissionMutation();
    ...
    swalWithBootstrapButtons
      ...
      .then((result) => {
        if (result.isConfirmed) {
          deleteAvisCommission(_id);
          swalWithBootstrapButtons.fire(
            "Supprimé!",
            "Avis de Commission a été supprimé.",
            "success"
          );
        } else if (result.dismiss === Swal.DismissReason.cancel) {
          swalWithBootstrapButtons.fire(
            "Annulé",
            "Avis de Commission est en sécurité :)",
            "error"
          );
        }
      });
  ...
  const columns = [
    ...
    {
      name: <span className="font-weight-bold fs-13">Stage</span>,
      cell: (row: any) => (
        <span>
          <strong>{row.type_stage.nom_fr}</strong>
        </span>
      ),
      sortable: true,
      width: "260px",
    ...
    <React.Fragment>
      <div className="page-content">
        <Container fluid={true}>
          <Breadcrumb
            title="Liste Avis de Commissions"
...
export default ListeAvisCommission;
