...
import { Message, useSendMessageMutation, useFetchRepliesByParentIdQuery, useTransferMessageMutation } from "features/messages/messagesSlice"
...
const SingleMessage = () => {
 ...
 useEffect(() => {
  ...
  const handleReplyFileChange = (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    if (files && files.length > 0) {
      ...
      Promise.all(fileArray.map(convertFileToBase64)).then((results) => {
        setReplyBase64Strings((prev) => [...prev, ...results.map((r) => r.base64String)]);
        setReplyExtensions((prev) => [...prev, ...results.map((r) => r.extension)]);
      });
    }
  };
  ...
  <Card.Header className="bg-primary text-white d-flex justify-content-between align-items-center">
    ...
    <small className="fw-light">{new Date(message.createdAt).toLocaleString()}</small>
  </Card.Header>
            <Card.Body>
...
<Card.Body className="pb-0">
      ...
      <Col md={6}>
        <div className="d-flex align-items-start">
          ...
          <div>
            ...
            <br />
            <small className="text-muted">{message.receiver.email}</small>
          </div>
        </div>
      </Col>
    </Row>
    ...
    <div className="mb-3 px-2">
      <div dangerouslySetInnerHTML={{ __html: message.content }} className="fs-6" />
    </div>

    {/* Attachments */}
    <div className="mb-3 px-2">
      <strong><i className="bi bi-paperclip me-1"></i> Pièces jointes:</strong>
        ...
        {message.attachments?.length > 0 ? (
          message.attachments.map((filename:any, idx:any) => {
            ...
            return (
              <Button
                ...
                onClick={() => handleOpenModal(fileUrl)}
              >
                <i className="bi bi-file-earmark-text me-2" /> Fichier {idx + 1}
              </Button>
            );
          })
        ) : (
          <span className="text-muted">Aucune</span>
                ...
                <div >
                  ...
                  {isLoading ? (
                    ...
                    {reply.attachments && reply.attachments.length > 0 ? (
                      reply.attachments.map((filename: string, index: number) => {
                        // Construct file URL
                        const fileUrl = `${process.env.REACT_APP_API_URL}/files/messagerieFiles/${filename}`;
                ...
                <div className="card-footer  d-flex justify-content-between">
                  ...
                  <div>
                    {/* Reply Button */}
                    <Button variant="outline-primary" onClick={() => setShowReplyModal(true)}>
  <i className="bi bi-reply-fill me-1" /> Répondre
</Button>
                    <Button variant="success" className="ms-2" onClick={() => handleTransferMessage(message)}>
  ...
  <Modal.Header closeButton>
    <Modal.Title>Répondre à {message.sender.nom_fr} {message.sender.prenom_fr}</Modal.Title>
  </Modal.Header>
  <Modal.Body>
  {/* To Field */}
  <div className="mb-2">
  ...
  {/* File Upload */}
  <label className="form-label"><strong>Pièces jointes:</strong></label>
  <input type="file" multiple onChange={handleReplyFileChange} className="form-control mb-3" />
      ...
      return (
        <div key={index} className="border rounded p-2" style={{ maxWidth: '200px' }}>
          ...
          <div className="text-center">
            <Button variant="outline-danger" size="sm" onClick={() => removeReplyFile(index)}>Supprimer</Button>
          </div>
        </div>
      );
    })}
  ...
  <Modal.Footer>
    <Button variant="secondary" onClick={() => setShowReplyModal(false)}>Annuler</Button>
    <Button variant="primary" onClick={handleReplySubmit}>Envoyer</Button>
      ...
      <Modal show={showTransferModal} onHide={() => setShowTransferModal(false)}>
        ...
        <Modal.Body>
          <Form.Select
            onChange={(e) => {
              const selected = allUsers.find((user) => user._id === e.target.value);
