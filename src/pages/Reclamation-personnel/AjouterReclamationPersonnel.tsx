...
import { useAddReclamationPersonnelMutation, Reclamation } from "features/reclamationPersonnel/reclamationPersonnelSlice";
...
const AjouterReclamationPersonnel = () => {
  document.title = "Ajouter Reclamation Personnel | Smart University";
  const navigate = useNavigate();
  const user = useSelector((state: RootState) => selectCurrentUser(state));
  const [addReclamationPersonnel] = useAddReclamationPersonnelMutation();
  ...
  const onSubmitReclamationPersonnel = async (e: React.FormEvent<HTMLFormElement>) => {
    e.preventDefault();
    try {
      await addReclamationPersonnel(formData).unwrap();
      notify();
                  ...
                  <Card.Header>
                    <div className="d-flex">
                      ...
                      <div className="flex-grow-1">
                        <h5 className="card-title">Nouvelle reclamation etudiant</h5>
                      </div>
                    </div>
                  </Card.Header>
                  <Card.Body></Card.Body>
                  <div className="mb-3">
                    <Form className="tablelist-form" onSubmit={onSubmitReclamationPersonnel}>
                      ...
                      <Form.Group className="mb-3">
                        ...
                        <Select
                          options={personnel.map(c => ({ value: c._id,  label: `${c.prenom_fr} ${c.nom_fr}` }))}
    ...
    >
      Supprimer la vidéo
                      ...
                      <div className="text-end">
                        <Button variant="secondary" onClick={() => navigate("/reclamation-personnel/liste-reclamation-personnel")}>
                          Annuler
                        </Button>
...
export default AjouterReclamationPersonnel;